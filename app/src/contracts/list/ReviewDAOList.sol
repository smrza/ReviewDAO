...
contract ReviewDAOList is IReviewDAOListMetadata, ReentrancyGuard {
    ...
    modifier ensureMemberOfDAO() {
        ...
        require(
            _token.balanceOf(owner) >= _settings.getMembershipAmountRDT(),
            "You do not own enough RDT to proceed."
        );
        _;
    }
    ...
    modifier processRDTPayment(uint256 cost_) {
        ...
        address owner = msg.sender;
        require(
            _token.allowance(owner, address(this)) >= cost_,
            "Allowance too low."
        );
        require(
            _token.transferFrom(owner, address(this), cost_),
    ...
    function applyListing(
        ...
        string memory name_,
        string memory baseUri_
    ) external processRDTPayment(_settings.getListingPriceRDT() * 3) {
    ...
    {
        require(
            !listings[listingId_].challenged,
            "This listing is already challenged."
        );
        require(
            !listings[listingId_].whitelisted,
            "This listing is whitelisted."
        );
        require(
            listings[listingId_].timer >= block.timestamp,
    ...
    function upvoteListing(bytes32 listingId_) external ensureMemberOfDAO {
        require(
            listings[listingId_].whitelisted,
            "This is not a whitelisted subject."
        );
        uint256 id = listings[listingId_].statusId;
        require(
            !statuses[id].accountVoted[msg.sender],
            "You already voted for this listing."
        );
        ++statuses[id].votes;
    ...
    function resolveListing(bytes32 listingId_) external nonReentrant {
        ...
        bool result;
        if (listing.timer < block.timestamp && !listing.challenged) {
        ...
        else if (listing.challenged) {
            if (challenges[listing.challengeId].timer < block.timestamp) {
                ...
                if (poll.votesFor >= poll.votesAgainst) {
                    ...
                    require(
                        _token.transfer(
                            listing.creator,
                            challenges[listing.challengeId].challengerReward
                        ),
                        "Transfer failed."
                    );
                    require(
                        _token.transfer(
                            _ReviewDAO,
                            challenges[listing.challengeId].listingStake
                        ),
                        "Transfer failed."
                    );
                    listing.statusId = statusesIds;
                    emit _ListingStatus(statusesIds, 0);
                    ++statusesIds;
                    uint256 additional = challenges[listing.challengeId]
                        .challengerStake % poll.votesFor;
                    require(
                ...
                //listing lost
                else if (poll.votesAgainst > poll.votesFor) {
                    require(
                        _token.transfer(
                            challenges[listing.challengeId].challenger,
                            challenges[listing.challengeId].challengerReward
                        ),
                        "Transfer failed."
                    );
                    require(
                        _token.transfer(
                            _ReviewDAO,
                            challenges[listing.challengeId].challengerStake
                        ),
                        "Transfer failed."
                    );
                    nameRegistered[listingId_] = false;
                    uint256 additional = challenges[listing.challengeId]
                        .listingStake % poll.votesAgainst;
                    require(
                        _token.transfer(_ReviewDAO, additional),
                        "Transfer failed."
                    );
    ...
    function claimRewards() external nonReentrant {
        ...
        uint256 length = 0;
        for (
            uint256 i = voters[msg.sender].votedChallenges.length;
            i > 0;
            i--
        ) {
        ...
        require(
            voters[msg.sender].votedChallenges.length == 0,
        ...
        for (
            ...
            if (challenges[j].timer < block.timestamp) {
                ...
                if (
                    ...
                    rewardBalance +=
                        challenges[j].listingStake /
                        polls[j].votesAgainst;
                }
            }
        }
    ...
    function banishListing(bytes32 listingId_) external {
        require(
            statuses[listings[listingId_].statusId].votes <
                _settings.getBanishListingLimit()
        );
        ...
        emit _Banished(
            ...
            false,
            msg.sender,
            statuses[listing.statusId].votes
        );
    }
